...
public sealed class RealmPersistenceTests : IClassFixture<MongoDatabaseFixture>, IAsyncLifetime
{
    ...
    [Fact(DisplayName = "[infrastructure] - when filtering realms by client id, then only realms with that client id are returned")]
    public async Task WhenFilteringRealmsByClientId_ThenOnlyRealmsFromThatClientAreReturned()
    {
        /* arrange: create realm with a specific client id */
        var clientId = "test-client-id";

        var realm = _fixture.Build<Realm>()
            .With(realm => realm.Name, "HttpsRichardy")
            .With(realm => realm.ClientId, clientId)
            .With(realm => realm.IsDeleted, false)
            .Create();
        ...
        /* act: query realms filtered by client id and name */
        var result = await _realmCollection.GetRealmsAsync(filters, CancellationToken.None);
        var retrievedRealm = result.FirstOrDefault();

        /* assert: only realm with matching client id is returned */
        Assert.NotNull(retrievedRealm);

        Assert.Equal(realm.Id, retrievedRealm.Id);
        Assert.Equal(realm.Name, retrievedRealm.Name);
        Assert.Equal(clientId, retrievedRealm.ClientId);
    }

    [Fact(DisplayName = "[infrastructure] - when counting realms with filters, then count must reflect filtered records")]
    public async Task WhenCountingRealmsWithFilters_ThenCountMustReflectFilteredRecords()
    {
        /* arrange: create 10 realms with varied clientId and IsDeleted */
        ...
        {
            var realm = _fixture.Build<Realm>()
                .With(realm => realm.ClientId, index < 5 ? clientId1 : clientId2) // first 5 client1, last 5 client2
                .With(realm => realm.Name, $"realm{index}")
                .With(realm => realm.IsDeleted, index % 3 == 0) // every third realm is deleted
                .Create();
        ...
        /* act: count realms filtered by clientId1 and IsDeleted = false */
        var filters = RealmFilters.WithSpecifications()
            .WithClientId(clientId1)
            .WithIsDeleted(false)
            .Build();

        var filteredCount = await _realmCollection.CountAsync(filters, CancellationToken.None);
        var expectedCount = realms.Count(realm => realm.ClientId == clientId1 && !realm.IsDeleted);

        /* assert: expected count of realms for client */
